...
function typeinf_edge(method::Method, atypes::ANY, sparams::SimpleVector, needtree::Bool, optimize::Bool, cached::Bool, caller, world::UInt)
    ...
    if isa(caller, InferenceState) && (caller::InferenceState).inferred
...
function typeinf_world(method::Method, atypes::ANY, sparams::SimpleVector, needtree::Bool, world::UInt)
    return typeinf_edge(method, atypes, sparams, needtree, true, true, nothing, world)
end
...
function inlineable(f::ANY, ft::ANY, e::Expr, atypes::Vector{Any}, sv::InferenceState)
    ...
    (src, rettype, inferred) = typeinf_edge(method, metharg, methsp, sv, sv.world)
...
function return_type(f::ANY, t::ANY)
    ...
    for m in _methods(f, t, -1, world)
        _, ty, inferred = typeinf_world(m[3], m[1], m[2], false, world)
        !inferred && return Any
        rt = tmerge(rt, ty)
        rt === Any && break
    end
